...
export class ActivePrincipleService {
   ...
   * Prefix endpoint
   */
  private prefix: string = "inventories/active_principles";

  /**
   * Data Behaviour
   */
  private _activesPrinciples: BehaviorSubject<any[]> = new BehaviorSubject([]);
  ...
  get activesPrinciples$(): Observable<any[]> {
    return this._activesPrinciples.asObservable();
  }
  ...
  list(paginator: MatPagination): void {
    ...
    this.http.get(`${this.prefix}`, { params: paginator.httpParams() })
      ...
      .subscribe((response: PaginateResponseType) => {
        ...
        this._activesPrinciples.next(response.data)
      })
