...
export class CategoryService {
   ...
   * Prefix endpoint
   */
  private prefix: string = "security_guards";
  ...
  list(paginator: MatPagination): void {
    ...
    this.http.get(`${this.prefix}?with=turn`, { params: paginator.httpParams() })
      .pipe(
  ...
  listTurn(paginator: MatPagination): void {

    paginator.isLoading = true;

    this.http.get(`${this.prefix}/turns`, { params: paginator.httpParams() })
      .pipe(
        finalize(() => paginator.isLoading = false)
      )
      .subscribe((response: PaginateResponseType) => {
 ...
 */
  ...
  storeTurn(data: any): Observable<any> {

    return of(null).pipe(
      switchMap(() => {

        if (data.id)
          return this.http.put(`${this.prefix}/turns/${data.id}`, data);

        return this.http.post(`${this.prefix}/turns`, data);
