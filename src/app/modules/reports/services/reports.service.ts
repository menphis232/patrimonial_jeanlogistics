 ...
 */
  ...
  download(params): void {
   this.http.get(`reports/medical_consultations/get`, { responseType: 'blob', params })
     .subscribe((response: Blob) => {
      //  this.saveFile(response, `medical_consultation.pdf`);
 ...
 downloadEjecutivo(params): void {
  this.http.get(`reports/executive/get`, { responseType: 'blob', params })
    .subscribe((response: Blob) => {
      this.saveFile(response, `executive.pdf`);
    });
}


downloadAccidentes(params): void {
  this.http.get(`reports/accidents_per_area/get`, { responseType: 'blob', params })
    .subscribe((response: Blob) => {
      this.saveFile(response, `accidents.pdf`);
    });
}

downloadMedicament(params): void {
  this.http.get(`reports/medicine`, { responseType: 'blob', params })
    .subscribe((response: Blob) => {
      this.saveFile(response, `recetas.pdf`);
    });
}
